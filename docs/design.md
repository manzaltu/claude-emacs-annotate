...
| `claude-emacs-annotate-view.el` | Buffer minor mode, overlays, tint/highlight styles, inline thread boxes (buffer-wide and per-thread expand/collapse with fringe indicators), the global tag filter's thread hiding (filtered threads render nothing and are invisible to every command; their overlays survive only as position trackers), the flush engine and revert brackets. |
| `claude-emacs-annotate-table.el` | Project-wide `tabulated-list` of threads, rendered from the store with anchors resolved against on-disk content; local filters compose with the global tag filter, showing shown/total when rows are hidden. |
