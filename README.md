...
Annotated regions are tinted with a subtle yellowish hue (or highlighted — `claude-emacs-annotate-display-style`; `claude-emacs-annotate-tint-hue` and `claude-emacs-annotate-tint-saturation-percent` control the hue), with thread text rendered inline in a box below the region by default. Each region's last line carries a fringe indicator: `-` while the thread box is expanded, `+` while collapsed (`claude-emacs-annotate-fringe-indicator` moves or disables it). `M-x claude-emacs-annotate-toggle-inline-at-point` expands or collapses the thread at point (prompting when overlapping threads share the spot); `M-x claude-emacs-annotate-toggle-inline` switches the whole buffer between inline boxes and highlight-only, resetting per-thread toggles; `M-x claude-emacs-annotate-refresh` rebuilds a buffer's overlays from the store if they ever look stale.

`M-x claude-emacs-annotate-filter-by-tag` narrows every view to one annotation set: threads without the chosen tag disappear entirely — no region tint, no inline boxes, no fringe indicator, no table rows — and no command reaches them: at-point commands treat them as absent, navigation skips their regions, and the jump prompt doesn't offer them. While a filter is active, the annotations table's mode line reads shown/total alongside the tag; entering nothing at the prompt clears the filter. Only the display is filtered — every thread stays persisted and addressable through the API.
